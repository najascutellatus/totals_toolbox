...
addpath(genpath('/Users/hroarty/Documents/MATLAB/HFR_RealTime/totals_toolbox'))
...
dbConn = connect2TotalsDB; % connect to the totals/radial database
...
vars = fieldnames(rangeVars); % get field names of each variable

for varInd = 1:length(vars)
    variable = vars{varInd};
    
      if strcmpi(SystemString, variable)
          systemType = rangeVars.(variable); % get the corresponding #
      else
          continue
      end
end
...
result = fetchDatafromDB(dbConn, dbStatement); % see if hfr system type is in database
...
radials2totals(systemType, dbConn,...
    'startTime', datenum(2015,6,1,0,0,0),...
    'endTime', datenum(2015,6,2,0,0,0),...
    'pattSelect', 3,...
    'region', 7,...
