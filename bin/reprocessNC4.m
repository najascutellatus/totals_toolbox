...
start_time = datenum(2009, 6, 6, 6, 0, 0);
...
for h = 1:length(dtime)
    file_name = ['/home/codaradm/data/totals/maracoos/oi/ascii/5MHz/OI_MARA_' datestr(dtime(h), 'yyyy_mm_dd_HH00')];
    file_name2 = ['/home/codaradm/data/totals/maracoos/oi/nc/5MHz/RU_5MHz)' datestr(dtime(h), 'yyyy_mm_dd_HH00')];
    if exist(file_name2, 'file')
        continue
    else
        ingestCodar(2, file_name)
    end
end
